...
function handleLogs() {
  ...
  if (logs) {
    ...
    if (accessLogging) {
      ...
      // If customAccessLoggingArn provided, use it instead of cloudwatch log group
      if (logs.customAccessLoggingArn)
        resourceArn = logs.customAccessLoggingArn;
...
function removeAccessLoggingLogGroup() {
  ...
  let customAccessLoggingArn;

  if (accessLogging) {
    customAccessLoggingArn = accessLogging.customAccessLoggingArn;
  ...
  // Also, if customAccessLoggingArn provided (for kinesis, firehose, etc), not using log group
  if ((!accessLogging) || customAccessLoggingArn) {
    return this.provider
